...
export const AttachmentUploader = ({ ticketId }) => {
  ...
  return (
    <Card className="glass-card mt-4" style={{ background: 'var(--card-bg)' }}>
      <Card.Header className="bg-transparent border-bottom border-secondary py-3 d-flex justify-content-between align-items-center">
        <h5 className="mb-0 fw-bold d-flex align-items-center gap-2" style={{ color: 'var(--text-dark)' }}>
          <Paperclip size={20} style={{ color: '#10b981' }} /> Attachments ({attachments.length})
        </h5>
        <div>
      ...
      <Card.Body>
        ...
        {uploading && (
          <div className="mb-3">
            <small className="d-block mb-1 fw-bold" style={{ color: '#8b949e' }}>Uploading to S3 Bucket...</small>
        ...
        {attachments.length === 0 ? (
          ...
          <ListGroup variant="flush" className="bg-transparent">
            {attachments.map((att) => (
              <ListGroup.Item
                key={att.id}
                className="border border-secondary text-white d-flex justify-content-between align-items-center py-2 px-3 rounded-2 mb-2"
                style={{ background: 'rgba(255, 255, 255, 0.02)' }}
              >
                <div className="d-flex align-items-center gap-2 text-truncate me-3">
                  <FileText size={18} style={{ color: '#10b981' }} className="flex-shrink-0" />
                  <div>
                    <div className="fw-semibold text-truncate" style={{ maxWidth: '300px', color: 'var(--text-dark)' }}>{att.fileName}</div>
                    <small style={{ color: '#8b949e' }}>{formatFileSize(att.fileSize)} • Uploaded by {att.uploadedBy?.firstName}</small>
                  </div>
                </div>
