...
# 7. CloudFront CDN Module (Bypassed due to account verification restrictions)
# module "cloudfront" {
...
resource "aws_lambda_permission" "allow_s3" {
  ...
  source_arn    = module.s3.attachment_bucket_arn
}

resource "aws_s3_bucket_notification" "bucket_notification" {
  bucket = module.s3.attachment_bucket_name

  lambda_function {
    lambda_function_arn = module.lambda.function_arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "tickets/"
  }
