          ...
          {comments.length === 0 ? (
            ...
            comments.map((comment) => (
              <div
                ...
                style={!comment.internal ? { background: 'rgba(255, 255, 255, 0.03)' } : {}}
              >
                <div className="d-flex justify-content-between align-items-center mb-2">
                  <div className="d-flex align-items-center gap-2">
                    <div
                      className="rounded-circle text-dark d-flex align-items-center justify-content-center fw-bold small shadow-sm"
                      style={{ width: 28, height: 28, background: 'linear-gradient(135deg, #10b981, #34d399)' }}
                    >
                      {comment.user?.firstName ? comment.user.firstName.charAt(0) : 'U'}
                    </div>
                    <span className="fw-bold" style={{ color: 'var(--text-dark)' }}>
                  ...
                  <small className="d-flex align-items-center gap-1" style={{ color: '#8b949e' }}>
                    <Clock size={12} /> {new Date(comment.createdAt).toLocaleString()}
                  </small>
                </div>
                <div className="small ms-4" style={{ whiteSpace: 'pre-wrap', color: 'var(--text-main)', fontWeight: '500' }}>
                  {comment.content}
                </div>
              </div>
            ))
