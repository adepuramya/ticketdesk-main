  ...
  return (
    <Card className="glass-card glass-card-interactive mb-3">
      <Card.Body>
        ...
        <Card.Title as={Link} to={`/tickets/${ticket.id}`} className="text-decoration-none text-white fw-bold fs-5 mb-2 d-block hover-underline">
          {ticket.title}
        </Card.Title>

        <Card.Text className="small text-truncate-2 mb-3" style={{ maxHeight: '2.8rem', overflow: 'hidden', color: '#8b949e' }}>
          {ticket.description}
        </Card.Text>

        <div className="d-flex flex-wrap align-items-center justify-content-between pt-2 border-top border-secondary small" style={{ color: '#8b949e' }}>
          ...
          <Link to={`/tickets/${ticket.id}`} className="d-flex align-items-center gap-1 text-decoration-none fw-semibold" style={{ color: '#10b981' }}>
