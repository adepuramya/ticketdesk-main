...
public class DashboardServiceImpl implements DashboardService {
    ...
    public DashboardStatsDto getDashboardStats(String username) {
        ...
        if (user.getRole().getName() == ERole.ROLE_EMPLOYEE) {
            ...
            totalTickets = ticketRepository.countByCreatedById(userId);
            openTickets = ticketRepository.countByCreatedByIdAndStatus(userId, TicketStatus.OPEN);
            inProgressTickets = ticketRepository.countByCreatedByIdAndStatus(userId, TicketStatus.IN_PROGRESS);
            resolvedTickets = ticketRepository.countByCreatedByIdAndStatus(userId, TicketStatus.RESOLVED);
            closedTickets = ticketRepository.countByCreatedByIdAndStatus(userId, TicketStatus.CLOSED);

            statusRaw = ticketRepository.countTicketsGroupByStatusForUser(userId);
            priorityRaw = ticketRepository.countTicketsGroupByPriorityForUser(userId);
            categoryRaw = ticketRepository.countTicketsGroupByCategoryForUser(userId);
        } else {
